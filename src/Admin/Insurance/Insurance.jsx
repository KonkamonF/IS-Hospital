...
    
    // Remaining amount that is estimated to be covered by insurance/needs approval
...
                <h3 className="text-xl font-bold text-[#2155CD] mb-4">กำหนดจำนวนเงินที่เบิกจ่ายได้</h3>
                <p className="text-sm text-gray-600 mb-4">ยอดที่ขอเบิกจ่าย: <span className="font-semibold">{formatCurrency(maxAmount)}</span></p>
                
...
                />
                {amount > maxAmount && (
                    <p className="text-xs text-red-500 mt-1">⚠️ ยอดเงินที่อนุมัติไม่ควรเกินยอดที่ขอเบิกจ่าย</p>
                )}
...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md transform transition-all">
                <h3 className="text-xl font-bold text-red-600 mb-4">สาเหตุที่ไม่อนุมัติการเบิกจ่าย</h3>
                
...
    // Logic for displaying errors or handling empty state
    if (!selectedData) {
...
        console.log(`อนุมัติเบิกจ่ายเต็มจำนวน: ${formatCurrency(requestedClaimAmount)}`);
        // TODO: Implement API call to approve the full requested amount
        alert(`🟢 อนุมัติเบิกจ่ายเต็มจำนวน ${formatCurrency(requestedClaimAmount)} สำเร็จ!`);
    };

    const handleApproveCustom = (amount) => {
        console.log(`อนุมัติเบิกจ่ายแบบกำหนดเอง: ${formatCurrency(amount)}`);
        // TODO: Implement API call to approve the custom amount
        setShowCustomApprovalModal(false);
        alert(`🟠 อนุมัติเบิกจ่ายแบบกำหนดเองเป็น ${formatCurrency(amount)} สำเร็จ!`);
    };

    const handleReject = (reason) => {
        console.log(`ไม่อนุมัติ: สาเหตุ -> ${reason}`);
        // TODO: Implement API call to reject the claim with the reason
...
                                ${requestedClaimAmount > 0 ? 'bg-red-600 text-white' : 'bg-green-600 text-white'}`}
                            >
...
                    {/* --- NEW SECTION: Claim Approval Actions --- */}
                    <div className="bg-white p-6 rounded-xl shadow-xl border-t-4 border-gray-300">
                        <h2 className="text-xl font-bold text-gray-800 mb-4">การดำเนินการเบิกจ่าย</h2>
                        
                        {/* ยอดที่ขอเบิกจ่าย (Requested Claim Amount) */}
                        <div className="mb-6 p-4 bg-blue-50 border-l-4 border-[#2155CD] rounded-lg flex justify-between items-center">
                            <span className="text-lg font-semibold text-gray-700">ยอดที่ขอเบิกจ่าย (Claim Request)</span>
...
                                className="flex-1 px-6 py-3 bg-green-600 text-white font-bold rounded-xl shadow-lg hover:bg-green-700 transition transform hover:scale-[1.01] active:scale-100"
                            >
...
                                className="flex-1 px-6 py-3 bg-yellow-500 text-white font-bold rounded-xl shadow-lg hover:bg-yellow-600 transition transform hover:scale-[1.01] active:scale-100"
                            >
                                💰 อนุมัติเบิกจ่ายแบบกำหนดจำนวนเอง
                            </button>

                            {/* Button 3: Reject */}
                            <button 
                                onClick={() => setShowRejectionModal(true)}
                                className="px-6 py-3 bg-red-600 text-white font-bold rounded-xl shadow-lg hover:bg-red-700 transition transform hover:scale-[1.01] active:scale-100"
                            >
                                ❌ ไม่อนุมัติ
                            </button>
                        </div>
